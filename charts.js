...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var otu_label = selectedSample.otu_labels;
    ...
    var yticks =  otu_id.sort((a,b) => 
      a.sample_values - b.sample_values).slice(0,10).reverse()
      .map(function(addOtu) {
        return "OTU "+ addOtu
      });

    var xticks =  sample_values.sort((a,b) => 
    a.sample_values - b.sample_values).slice(0,10).reverse();

    var hoverLables =  otu_label.sort((a,b) => 
    a.sample_values - b.sample_values).slice(0,10).reverse();
    ...
    var metDataArray = metData.filter(samplesObj => samplesObj.id == sample);
    
    //  5. Create a variable that holds the first sample in the array.
    
    var selectedmetData = metDataArray[0];

    var wfreqGauge = selectedmetData.wfreq
